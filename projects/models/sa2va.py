...
class Sa2VAModel(BaseModel):
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        valid_indices = [i for i, item in enumerate(pred_embeddings_list_) if len(item) > 0]
        ...
        if len(valid_indices) == 0:
            ...
            pred_embeddings = hidden_states[:, :5].flatten(0, 1) + _zero
            pred_embeddings_list = [pred_embeddings[i*5:(i+1)*5] for i in range(len(frames_per_batch))]
        else:
            pred_embeddings_list = [pred_embeddings_list_[i] for i in valid_indices]
            frames_per_batch = [frames_per_batch[i] for i in valid_indices]
            g_pixel_values = [g_pixel_values[i] for i in valid_indices]
            gt_masks = [gt_masks[i] for i in valid_indices]
